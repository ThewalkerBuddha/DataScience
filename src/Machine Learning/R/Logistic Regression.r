...
summary(modeldata$Housing)modeldata$Housing[modeldata$Housing==""]<-"A152"
head(modeldata)
#------------------------------------------------------------------------------------------------
#variable reduction
table(modeldata$Num_Dependents,modeldata$Default_On_Payment)
aggregate(modeldata$Num_Dependents,by=list(modeldata$Num_Dependents,modeldata$Default_On_
Payment),length)
...
aggregate(modeldata$Job_Status,by=list(modeldata$Job_Status,modeldata$Default_On_Payment),leng
th)
...
lol<-table(modeldata$Purpose_Credit_Taken,modeldata$Default_On_Payment)
aggregate(modeldata$Purpose_Credit_Taken,by=list(modeldata$Purpose_Credit_Taken,modeldata$Def
ault_On_Payment),length)#if we want all rows
myplot<-function(n)
{
...
myplot(10)
modeldata$dummy_p_c_t_l<-
ifelse(modeldata$Purpose_Credit_Taken=="P41"|modeldata$Purpose_Credit_Taken=="P43"|modeldat
a$Purpose_Credit_Taken=="P48",1,0)
modeldata$dummy_p_c_t_h<-
ifelse(modeldata$Purpose_Credit_Taken=="P49"|modeldata$Purpose_Credit_Taken=="P40"|modeldat
a$Purpose_Credit_Taken=="P45"|modeldata$Purpose_Credit_Taken=="P50"|modeldata$Purpose_Cre
dit_Taken=="P46",1,0)
...
ifelse(modeldata$Credit_History=="A30"|modeldata$Credit_History=="A31",1,0)
...
data1<-lol4[1:n,2,drop=F]sum1=data+data1
per=(data1/sum1)*100
per
}
myplot4(5)
modeldata$dummy_Years_At_Present_Employment_high<-
ifelse(modeldata$Years_At_Present_Employment=="E71"|modeldata$Years_At_Present_Employment=
="E72",1,0)
modeldata$Yearsdummy_At_Present_Employment_medium<-
...
modeldata$dummy_maritial_s_g<-
ifelse(modeldata$Marital_Status_Gender=="A91"|modeldata$Marital_Status_Gender=="A92",1,0)
...
table(modeldata$Foreign_Worker,modeldata$Default_On_Payment)modeldata$dummy_f_w<-ifelse(modeldata$Foreign_Worker=="A201",1,0)
View(modeldata)
...
vif_output<-
lm(Default_On_Payment~dummy_j_s_A171+dummy_j_s_A172+dummy_j_s_A173+dummy_p_c_t_h+du
mmy_p_c_t_l+dummy_s_c_a_H+dummy_s_c_a_M+dummy_Credit_History_high+dummy_Credit_Histor
y_low+dummy_Years_At_Present_Employment_high+dummy_Credit_History_low+dummy_maritial_s_
g+dummy_other_d_g+dummy_f_w+dummy_Age_group+dummy_credit_amount+dummy_Current_Add
ress_yrs+Duration_in_Months,data=modeldata)
#view vif outputvif(vif_output)
#here we can see that for a172 and a173 the vif is >2.so 1 variable to be removed
#in the next step what iam doing is regressing both variables seperately and retaining one variable with
highest r square
...
vif_output<-
lm(Default_On_Payment~dummy_j_s_A171+dummy_j_s_A172+dummy_p_c_t_h+dummy_p_c_t_l+du
mmy_s_c_a_H+dummy_s_c_a_M+dummy_Credit_History_high+dummy_Credit_History_low+dummy_Y
ears_At_Present_Employment_high+dummy_Credit_History_low+dummy_maritial_s_g+dummy_other_
d_g+dummy_f_w+dummy_Age_group+dummy_credit_amount+dummy_Current_Address_yrs+Duration
_in_Months,data=modeldata)
vif(vif_output)
...
vif_output<-
lm(Default_On_Payment~dummy_j_s_A171+dummy_j_s_A172+dummy_p_c_t_l+dummy_s_c_a_H+du
mmy_s_c_a_M+dummy_Credit_History_high+dummy_Credit_History_low+dummy_Years_At_Present_
Employment_high+dummy_Credit_History_low+dummy_maritial_s_g+dummy_other_d_g+dummy_f_w
+dummy_Age_group+dummy_credit_amount+dummy_Current_Address_yrs+Duration_in_Months,data
=modeldata)
vif(vif_output)#here we can see that all vif values are less than 1.5 so we can peoceed further
#-------------------------------------------------------------------------------------------------------------------
#Logistic regression model building
#glm-generalized inear model
#family=binomial specifies it is binomial logistic regression model
logreg_output<-
glm(Default_On_Payment~dummy_j_s_A171+dummy_j_s_A172+dummy_p_c_t_l+dummy_s_c_a_H+du
mmy_s_c_a_M+dummy_Credit_History_high+dummy_Credit_History_low+dummy_Years_At_Present_
Employment_high+dummy_Credit_History_low+dummy_maritial_s_g+dummy_other_d_g+dummy_f_w
+dummy_Age_group+dummy_credit_amount+dummy_Current_Address_yrs+Duration_in_Months,fami
ly=binomial(logit),data=modeldata)
summary(logreg_output,direction="forward")
#removing insignificant variables
logreg_output<-
glm(Default_On_Payment~dummy_j_s_A171+dummy_j_s_A172+dummy_s_c_a_H+dummy_s_c_a_M+d
ummy_Credit_History_high+dummy_Credit_History_low+dummy_Years_At_Present_Employment_high
+dummy_Credit_History_low+dummy_maritial_s_g+dummy_other_d_g+dummy_f_w+dummy_Age_gro
up+dummy_Current_Address_yrs+Duration_in_Months,family=binomial(logit),data=modeldata)
summary(logreg_output,direction="forward")
...
getwd()
#--------------------------------------------------------------------------------------------------------------
#Logistic Regression-Model Validation#outlier capping
...
aggregate(testdata$Num_Dependents,by=list(testdata$Num_Dependents,testdata$Default_On_Payme
nt),length)
...
#converting categorial value to dummyvariables
#dummy variables for job status
testdata$dummy_j_s_A171<-ifelse(testdata$Job_Status=="A171",1,0)testdata$dummy_j_s_A172<-ifelse(testdata$Job_Status=="A172",1,0)
testdata$dummy_j_s_A173<-ifelse(testdata$Job_Status=="A173",1,0)
#dummy variables for purpose credit taken
lol<-table(testdata$Purpose_Credit_Taken,testdata$Default_On_Payment)
aggregate(testdata$Purpose_Credit_Taken,by=list(testdata$Purpose_Credit_Taken,testdata$Default_O
n_Payment),length)
#if we want all rows
myplot<-function(n)
{
...
myplot(10)
testdata$dummy_p_c_t_l<-
ifelse(testdata$Purpose_Credit_Taken=="P41"|testdata$Purpose_Credit_Taken=="P43"|testdata$Purp
ose_Credit_Taken=="P48",1,0)
testdata$dummy_p_c_t_h<-
ifelse(testdata$Purpose_Credit_Taken=="P49"|testdata$Purpose_Credit_Taken=="P40"|testdata$Purp
ose_Credit_Taken=="P45"|testdata$Purpose_Credit_Taken=="P50"|testdata$Purpose_Credit_Taken==
"P46",1,0)
#dummy variables for status checking account
lol2<-table(testdata$Status_Checking_Accnt,testdata$Default_On_Payment)
myplot2<-function(n){
data<-lol2[1:n,1,drop=F]
data1<-lol2[1:n,2,drop=F]
sum1=data+data1
...
#dummy variables for credit history
lol3<-table(testdata$Credit_History,testdata$Default_On_Payment)
myplot3<-function(n)
{
data<-lol3[1:n,1,drop=F]
data1<-lol3[1:n,2,drop=F]
...
myplot3(5)
testdata$dummy_Credit_History_high<-
ifelse(testdata$Credit_History=="A30"|testdata$Credit_History=="A31",1,0)
testdata$dummy_Credit_History_low<-ifelse(testdata$Credit_History=="A34",1,0)#dummy variables for years at present employement
lol4<-table(testdata$Years_At_Present_Employment,testdata$Default_On_Payment)
myplot4<-function(n)
{
data<-lol4[1:n,1,drop=F]
...
myplot4(5)
testdata$dummy_Years_At_Present_Employment_high<-
ifelse(testdata$Years_At_Present_Employment=="E71"|testdata$Years_At_Present_Employment=="E7
2",1,0)
testdata$Yearsdummy_At_Present_Employment_medium<-
ifelse(testdata$Years_At_Present_Employment=="E73",1,0)
#dummy variables for maritial status
table(testdata$Marital_Status_Gender,testdata$Default_On_Payment)
testdata$dummy_maritial_s_g<-
ifelse(testdata$Marital_Status_Gender=="A91"|testdata$Marital_Status_Gender=="A92",1,0)
head(testdata$dummy_maritial_s_g)
#dummy variables for other debtors
table(testdata$Other_Debtors_Guarantors,testdata$Default_On_Payment)
testdata$dummy_other_d_g<-ifelse(testdata$Other_Debtors_Guarantors=="A103",0,1)#dummy variable for housing
...
lm(Default_On_Payment~dummy_j_s_A171+dummy_j_s_A172+dummy_p_c_t_l+dummy_s_c_a_H+du
mmy_s_c_a_M+dummy_Credit_History_high+dummy_Credit_History_low+dummy_Years_At_Present_
Employment_high+dummy_Credit_History_low+dummy_maritial_s_g+dummy_other_d_g+dummy_f_w+dummy_Age_group+dummy_credit_amount+dummy_Current_Address_yrs+Duration_in_Months,data
=modeldata)
vif(vif_output)
summary(logreg_output,direction="forward")
#estimate the probability of each record
...
#.........................function................................................
View(modeldata[modeldata$Purpose_Credit_Taken=="P40",c(2,3)])
View(modeldata)
lol<-table(modeldata$Purpose_Credit_Taken,modeldata$Default_On_Payment)
colnames(lol)<-c("ND","D")
#function....for calculating percentage for default with respect to purpose credit taken
myplot<-function()
{
data<-lol[1,1]
data1<-lol[1,2]
sum1=data+data1
per=(data1/sum1)*100per
}
myplot()
lol[1:1,1,drop=F]
#if we want all rows
...
myplot1(2,2)install.packages("car")